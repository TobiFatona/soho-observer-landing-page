...
export default function Hero() {
  return (
    <section className="relative min-h-screen flex items-center bg-parchment overflow-x-hidden">
      <div className="w-full max-w-[1400px] mx-auto px-[6vw] flex flex-col md:flex-row items-center gap-12 lg:gap-28 pt-24 pb-16 md:pt-20 md:pb-0">
        ...
        <div className="flex-1 flex flex-col items-center md:items-start text-center md:text-left min-w-0">
          ...
          <div className="mb-6">
            <motion.span
              {...fadeUp(0.15)}
              className="font-display italic font-light text-charcoal leading-[0.91] block"
            ...
            <motion.span
              {...fadeUp(0.35)}
              className="font-display italic font-light text-gold leading-[0.91] block"
              style={{ fontSize: 'clamp(48px, 6vw, 100px)' }}
            >
              Yours.
            </motion.span>
          ...
          >
            Welcome to Soho Observer — the AI app that lets you photograph any outfit or upload a saved image, identifies every piece by brand and price point, and shows you exactly where to find it. Curated alternatives at every budget. In under a minute.
          </motion.p>

          <motion.div {...fadeUp(0.6)} className="flex flex-col gap-3 mb-9 w-full max-w-[440px] mx-auto md:mx-0">
            ...
            <div className="flex items-center justify-center md:justify-start">
              <div className="inline-flex items-center gap-2 bg-charcoal/5 border border-charcoal/12 rounded-full px-4 py-1.5">
                ...
                <span className="font-sans text-[0.72rem] text-charcoal/80 font-medium">Coming soon to iOS</span>
              </div>
            </div>
          </motion.div>

          <motion.div {...fadeUp(0.7)} className="flex items-stretch justify-center md:justify-start">
            {stats.map(({ number, label }, i) => (
              <div
                key={label}
                className={`flex flex-col items-start py-1 ${
                  i > 0 ? 'border-l border-card ml-6 pl-6 sm:ml-10 sm:pl-10' : ''
                }`}
              >
                <span className="font-display-sc text-gold leading-none mb-1.5" style={{ fontSize: '0.48rem', letterSpacing: '0.1em' }}>✦</span>
                <span
                  className="font-display italic text-charcoal leading-none"
        ...
        >
          ...
          {/* Annotation 1 */}
          <div className="absolute z-20 flex flex-col items-end gap-1.5" style={{ left: 0, top: 162 }}>
            <span
              className="font-display-sc text-charcoal uppercase text-right leading-loose"
              style={{ fontSize: '0.48rem', letterSpacing: '0.22em' }}
            >
              Scan any<br />outfit
            </span>
            <svg width="116" height="22" viewBox="0 0 116 22" fill="none">
              <path d="M3,11 C26,1 90,21 113,11" stroke="#C4A96E" strokeWidth="1.2" strokeLinecap="round" />
              <path d="M108,6 L114,11 L108,16" stroke="#C4A96E" strokeWidth="1.2" strokeLinecap="round" strokeLinejoin="round" />
            </svg>
          </div>

          {/* Annotation 2: result card */}
          <div className="absolute z-20 flex flex-col items-start" style={{ right: 0, bottom: 10 }}>
            <svg width="46" height="42" viewBox="0 0 46 42" fill="none" className="self-start ml-4">
              <path d="M44,40 C32,24 10,26 3,8" stroke="#C4A96E" strokeWidth="1.2" strokeLinecap="round" />
              <path d="M0,2 L4,10 L10,5" stroke="#C4A96E" strokeWidth="1.2" strokeLinecap="round" strokeLinejoin="round" />
            </svg>
            <div
            ...
            >
              <div
                className="font-display-sc text-gold uppercase"
                style={{ fontSize: '0.44rem', letterSpacing: '0.22em', marginBottom: 10 }}
              >
                Identified
              </div>
              {[['01', 'Tie-Dye Knit'], ['02', 'Cargo Trousers'], ['03', 'Air Force 1']].map(([n, label]) => (
                <div key={label} style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 7 }}>
                  <span className="font-display-sc" style={{ color: 'rgba(255,255,255,0.18)', fontSize: '0.38rem' }}>{n}</span>
                  <span className="font-sans" style={{ color: '#F5F2EF', fontSize: '0.66rem' }}>{label}</span>
                </div>
              ))}
              <div style={{ borderTop: '1px solid rgba(255,255,255,0.07)', paddingTop: 9, marginTop: 2 }}>
                <span className="font-sans text-gold" style={{ fontSize: '0.56rem' }}>View &amp; shop →</span>
              </div>
            </div>
          </div>
        </motion.div>
